...
func TestDeleteSourceWithYes(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"message": "Deleted", "id": "src-1"}`)
	}))
	defer srv.Close()
	...
	assert.Contains(t, buf.String(), "Deleted source src-1.")
}

func TestDeleteSourceQuiet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"message": "Deleted", "id": "src-1"}`)
	}))
...
func TestDeleteSourceJSONMode(t *testing.T) {
	var deleteCalled bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		deleteCalled = true
		w.Header().Set("Content-Type", "application/json")
