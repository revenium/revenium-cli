...
func TestDeleteSubscriberWithYes(t *testing.T) {
	...
	assert.Contains(t, buf.String(), "Deleted subscriber sub-1.")
}

func TestDeleteSubscriberQuiet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"message": "Deleted", "id": "sub-1"}`)
	}))
...
func TestDeleteSubscriberJSONMode(t *testing.T) {
	var deleteCalled bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		deleteCalled = true
		w.Header().Set("Content-Type", "application/json")
