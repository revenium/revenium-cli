...
func TestDeleteAnomalyWithYes(t *testing.T) {
	...
	assert.Contains(t, buf.String(), "Deleted anomaly anom-1.")
}

func TestDeleteAnomalyQuiet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"message": "Deleted", "id": "anom-1"}`)
	}))
...
func TestDeleteAnomalyJSONMode(t *testing.T) {
	var deleteCalled bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		deleteCalled = true
		w.Header().Set("Content-Type", "application/json")
