...
func TestListChartsEmpty(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `[]`)
	}))
	...
	require.NoError(t, err)
	assert.Contains(t, buf.String(), "No charts found.")
}

func TestListChartsJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `[{"id": "chart-1", "label": "Revenue Chart", "type": "bar", "created": "2026-01-01"}]`)
	}))
	...
	assert.Equal(t, "chart-1", result[0]["id"])
}

func TestListChartsEmptyJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `[]`)
	}))
